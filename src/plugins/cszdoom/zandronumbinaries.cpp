...
#define ZANDRONUM_BINARY_NAME "cszdoom.exe"
...
#ifndef Q_OS_MAC
#define ZANDRONUM_BINARY_NAME "cszdoom"
#else
#define ZANDRONUM_BINARY_NAME "csZDoom.app"
#define ZANDRONUM_APP_BUNDLE_BIN "/Contents/MacOS/cszdoom"
#endif
...
Message ZandronumClientExeFile::install(QWidget *parent)
{
		...
		QMessageBox::Yes | QMessageBox::No) == QMessageBox::Yes)
	{
		...
		if (path.isEmpty())
		{
			return Message::customError(tr("No testing directory specified for csZDoom"));
		}
		...
		// Now copy all .ini's. On Linux .ini's are kept in ~/.cszdoom so this will
...
QString ZandronumClientExeFile::pathToExe(Message& message)
{
	...
	else
	{
		...
		{
			error = tr("%1\ndoesn't contain csZDoom executable").arg(path);
			message = Message::customError(error);
			return QString();
		}
