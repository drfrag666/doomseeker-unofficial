...
#define TESTING_BINARY_URL_BASE "https://qzandronum.com/downloads/testing/%1/ZandroDev%1-%2"
...
Message ZandronumqClientExeFile::install(QWidget *parent)
{
	if (askToInstallTestingVersion(parent))
	{
		...
		if (path.isEmpty())
		{
			return Message::customError(tr("No testing directory specified for Zandronumq"));
		}
		...
		// Now copy all .ini's. On Linux .ini's are kept in ~/.zandronumq so this will
...
bool ZandronumqClientExeFile::askToInstallTestingVersion(QWidget *parent) const
{
	QString question;
	...
	if (d->isNonEmptyDir(testingVersionInstallPath()))
	{
		question = tr("<p>Installation of testing binaries for version %1 "
			...
			"<p>Do you want Doomseeker to extract Zandronumq files, potentially "
	...
	{
		question = tr("Zandronumq testing version %1 can be installed.\n\n"
	...
	QMessageBox box(icon, tr("Doomseeker - install Zandronumq testing version"), question,
...
QString ZandronumqClientExeFile::pathToExe(Message &message)
{
	...
	else
	{
		...
		{
			error = tr("No testing directory specified for Zandronumq");
		...
		QString binPath = path + '/' + ZANDRONUMQ_BINARY_NAME;
...
QString ZandronumqClientExeFile::testingVersion() const
{
...
QString ZandronumqClientExeFile::testingVersionInstallPath() const
{
	IniSection &config = *ZandronumqEnginePlugin::staticInstance()->data()->pConfig;
	QString path =  config["TestingPath"];
...
bool ZandronumqClientExeFile::downloadTestingBinaries(const QDir &destination, QWidget *parent)
{
	#ifdef Q_OS_DARWIN
	...
	ZandronumqVersion version(d->server->gameVersion());
	...
	gLog << tr("Downloading Zandronumq testing binary from URL: %1").arg(url.toString());
		...
		if (!archive.isNull())
		{
			for (int i = 0;; ++i)
			{
				...
				if (archive->extract(i, fileTargetPath))
				{
					...
					if (filename == ZANDRONUMQ_BINARY_NAME)
					{
...
bool ZandronumqClientExeFile::spawnTestingBatchFile(const QString &versionDir, QString &fullPathToFile, Message &message)
{
	QString binaryPath = versionDir + '/' + ZANDRONUMQ_BINARY_NAME;
	...
	#ifdef Q_OS_DARWIN
	content += "\"" + binaryPath + ZANDRONUMQ_APP_BUNDLE_BIN "\" \"$@\"\n";
	#else
