...
//------------------------------------------------------------------------------
#ifndef DOOMSEEKER_PLUGIN_ZANDRONUMQSERVER_H
#define DOOMSEEKER_PLUGIN_ZANDRONUMQSERVER_H
...
#define NUM_ZANDRONUMQ_GAME_MODES 16
#define NUM_ZANDRONUMQ_GAME_MODIFIERS 2
...
{
public:
	ZandronumqVersion(QString version);
	...
	bool operator> (const ZandronumqVersion &other) const;
...
class ZandronumqServer : public Server
{
	Q_OBJECT
...
public:
	...
	enum ZandronumqQueryFlags
	{
		...
		SQF_STANDARDQUERY =
			...
			SQF_ALL_DMFLAGS | SQF_SECURITY_SETTINGS |
			SQF_OPTIONAL_WADS | SQF_DEH | SQF_EXTENDED_INFO,

		SQF2_PWAD_HASHES = 0x00000001,

		SQF2_STANDARDQUERY = SQF2_PWAD_HASHES
	};

	ZandronumqServer(const QHostAddress &address, unsigned short port);
