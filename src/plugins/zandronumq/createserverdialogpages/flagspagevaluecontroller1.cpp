...
FlagsPageValueController::FlagsPageValueController(FlagsPage *flagsPage)
{
	...
	this->compatflags = 0;
	this->compatflags2 = 0;
	this->zandronumqCompatflags = 0;
	this->dmflags = 0;
	this->dmflags2 = 0;
	this->zandronumqDmflags = 0;
	...
	// LMSAllowedWeapons
	{
		...
		m.insert(ZandronumqGameInfo::LMSAW_Chainsaw,
			f->cbLMSChainsaw);
		m.insert(ZandronumqGameInfo::LMSAW_Pistol,
			f->cbLMSPistol);
		m.insert(ZandronumqGameInfo::LMSAW_Shotgun,
			f->cbLMSShotgun);
		m.insert(ZandronumqGameInfo::LMSAW_SuperShotgun,
			f->cbLMSSuperShotgun);
		m.insert(ZandronumqGameInfo::LMSAW_Chaingun,
			f->cbLMSChaingun);
		m.insert(ZandronumqGameInfo::LMSAW_Minigun,
			f->cbLMSMinigun);
		m.insert(ZandronumqGameInfo::LMSAW_RocketLauncher,
			f->cbLMSRocketLauncher);
		m.insert(ZandronumqGameInfo::LMSAW_GrenadeLauncher,
			f->cbLMSGrenadeLauncher);
		m.insert(ZandronumqGameInfo::LMSAW_PlasmaRifle,
			f->cbLMSPlasmaRifle);
		m.insert(ZandronumqGameInfo::LMSAW_Railgun,
			f->cbLMSRailgun);
	}
	...
	// LMSSpectatorSettings
	{
		...
		m.insert(ZandronumqGameInfo::LMSSS_TalkToActivePlayers,
			f->cbLMSSpectatorsCanTalkToActivePlayers);
		m.insert(ZandronumqGameInfo::LMSSS_ViewTheGame,
...
void FlagsPageValueController::convertNumericalToWidgets()
{
	...
	convertNumericalToCheckboxes(zandronumqCompatflagsCheckboxes, zandronumqCompatflags);
	convertNumericalToCheckboxes(dmflagsCheckboxes, dmflags);
	convertNumericalToCheckboxes(dmflags2Checkboxes, dmflags2);
	convertNumericalToCheckboxes(zandronumqDmflagsCheckboxes, zandronumqDmflags);
...
void FlagsPageValueController::convertWidgetsToNumerical()
{
	...
	convertCheckboxesToNumerical(zandronumqCompatflagsCheckboxes, zandronumqCompatflags);
	convertCheckboxesToNumerical(dmflagsCheckboxes, dmflags);
	convertCheckboxesToNumerical(dmflags2Checkboxes, dmflags2);
	convertCheckboxesToNumerical(zandronumqDmflagsCheckboxes, zandronumqDmflags);
	...
	flagsPage->leZandronumqDmflags->setText(QString::number(zandronumqDmflags));
	flagsPage->leCompatflags->setText(QString::number(compatflags));
	flagsPage->leCompatflags2->setText(QString::number(compatflags2));
	flagsPage->leZandronumqCompatflags->setText(QString::number(zandronumqCompatflags));
...
void FlagsPageValueController::convertToWidgetPlayers()
{
	...
	if ((zandronumqDmflags & Dmflags::ZADF_UNBLOCK_PLAYERS) == 0 &&
		(zandronumqDmflags & Dmflags::ZADF_UNBLOCK_ALLIES) == 0)
	{
...
void FlagsPageValueController::readFlagsFromTextInputs()
{
	...
	zandronumqDmflags = flagsPage->leZandronumqDmflags->text().toUInt();
	compatflags = flagsPage->leCompatflags->text().toUInt();
	compatflags2 = flagsPage->leCompatflags2->text().toUInt();
	zandronumqCompatflags = flagsPage->leZandronumqCompatflags->text().toUInt();
